...
ホストプロジェクトとコンポーネントの関係は，どちらの場合であっても共通です。

* ホストプロジェクトのソースコードを変更しない場合: コンポーネントの共有メソッドを`マニュアル`操作で実行し，サーバー側に常駐プロセスのストアドプロシージャーを開始します。メリットとして，ホストプロジェクトのソースコードを変更しないで済むことが挙げられます。たとえば，コンパイル版のアプリケーションをデプロイしている場合，再コンパイルしなくても良いので便利です。一方，サーバーアプリケーションを再起動するたびに，再度ストアドプロシージャーを開始しなければならないというデメリットがあります。
